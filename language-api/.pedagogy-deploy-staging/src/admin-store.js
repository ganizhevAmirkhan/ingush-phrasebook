...
const PEDAGOGY_KNOWLEDGE_FILES = {
  "ozdoev-1970": path.join(GRAMMAR_DIR, "ozdoev-1970-knowledge.json"),
  "iomabara-praktikum": path.join(GRAMMAR_DIR, "iomabara-praktikum-knowledge.json"),
  "ozdoev-ortography-2003": path.join(GRAMMAR_DIR, "ozdoev-ortography-2003-knowledge.json"),
  "hlanzara-ingush": path.join(GRAMMAR_DIR, "hlanzara-ingush-knowledge.json")
};
...
async function getInventory() {
  ...
  for (const [bookId, filePath] of Object.entries(PEDAGOGY_KNOWLEDGE_FILES)) {
    try {
      const json = await readJson(filePath);
      pedagogyBooks[bookId] = {
        sections: Array.isArray(json?.sections) ? json.sections.length : 0,
        stats: json?.stats || null,
        titleRu: json?.titleRu || null
      };
    } catch {
      pedagogyBooks[bookId] = { sections: 0, stats: null, titleRu: null };
    }
  }
  return {
...
async function getPedagogyKnowledgeMeta(bookId) {
  const filePath = PEDAGOGY_KNOWLEDGE_FILES[bookId];
  if (!filePath) return null;
  try {
    const json = await readJson(filePath);
    const sections = Array.isArray(json?.sections) ? json.sections : [];
...
async function getPedagogyKnowledgeSection(bookId, id) {
  const filePath = PEDAGOGY_KNOWLEDGE_FILES[bookId];
  if (!filePath) return null;
  try {
    const json = await readJson(filePath);
...
module.exports = {
  ...
  getPedagogyKnowledgeMeta,
  getPedagogyKnowledgeSection,
  PEDAGOGY_KNOWLEDGE_FILES
};
